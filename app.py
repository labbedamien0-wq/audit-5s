...
# Audio passive unlock via native HTML5
...
def trigger_vote_fx(step_num, vote_type="OUI"):
    ...
    # Single native HTML5 Audio Tag + Timestamp query param to force mobile replay without WebSocket components
    vote_html = (
        f"<audio autoplay='true' style='display:none;' src='data:audio/wav;base64,{CHIME_B64}#t={ts}'></audio>"
...
def trigger_final_save_fx():
    ...
    ts = int(time.time() * 1000)
    final_html = (
        f"<audio autoplay='true' style='display:none;' src='data:audio/wav;base64,{FANFARE_B64}#t={ts}'></audio>"
